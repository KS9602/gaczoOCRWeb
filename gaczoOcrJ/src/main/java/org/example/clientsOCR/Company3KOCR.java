...
import org.example.records.Company3KRow;
...
public class Company3KOCR extends BaseOCR<Company3KRow>{

    private static final int lineColor = -1842205;

    public Company3KOCR() throws IOException {
        super();
    }
    ...
    public String provideInvoicePattern() {
        return "Faktura VAT Korekta";
    }

    @Override
    public List<Company3KRow> getRows(PDDocument document, int pageIndex) throws IOException {
        ...
        Company3Stripper company3Stripper = new Company3Stripper();
        company3Stripper.setStartPage(pageIndex + 1);
        company3Stripper.setEndPage(pageIndex + 1);
        company3Stripper.getText(document);

        int lpY = (int) company3Stripper.lpY;
        if (lpY == 0){
            return List.of();
        }
        int lpX = (int) company3Stripper.lpX;

        List<Integer> rowYpos = calcY(img, lpX, lpY);
        log.info("Znaleziono pozycje Y :{}", rowYpos);

        List<Integer> rowXpos = clacX(img, lpX, lpY);
        log.info("Znaleziono pozycje X :{}",rowXpos);
    ...
    @Override
    public List<Company3KRow> buildRowRecords(PDPage page, List<List<String>> regionNames) throws IOException {
        areaStripper.extractRegions(page);

        List<Company3KRow> rowRecords = new ArrayList<>();
